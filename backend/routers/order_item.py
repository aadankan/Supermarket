...
def get_item(order_id: int, product_id: int, db: Session = Depends(get_db)):
...
def update_item(order_id: int, product_id: int, item: OrderItemUpdate, db: Session = Depends(get_db)):
...
def delete_item(order_id: int, product_id: int, db: Session = Depends(get_db)):
    existing = crud_order_item.get_order_item(db, order_id, product_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Order item not found")
